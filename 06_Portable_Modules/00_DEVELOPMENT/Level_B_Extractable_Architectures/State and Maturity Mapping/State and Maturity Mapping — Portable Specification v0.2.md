# State and Maturity Mapping — Portable Specification v0.2
...
A missing subfunction does not automatically make the whole assessed function PARTIAL. Depending on the declared assessment boundary it may instead be a maturity deficit, a partial-function deficit, or a separate required function.
...
Where evidence shows a material context, dependency or capability change since review, an old SUFFICIENT claim must not be presented as unqualified current SUFFICIENT without revalidation. The prior state remains historically valid as a prior assessment; current state may be UNKNOWN, review-due, or another host-declared bounded state.
...
Freeze Blind Transfer Test 002 and its expected-findings key before obtaining an independent response. The second test should use a materially different non-Concord domain and directly pressure the bounded clarifications introduced after BTT-001.
